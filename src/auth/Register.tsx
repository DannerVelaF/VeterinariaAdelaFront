...
import {
    ...
    registrarUsuario, verificarDocumentoExistente, verificarUsuarioExistente,
} from '../service/api';
...
function Register() {
        ...
        try {
            ...
            if (response.existe) {
                ...
                if (response.sin_usuario) {
                    ...
                    setMensajeDocumento('Documento verificado - Completa tu usuario y contraseña');
                    setCamposBloqueados(true);
                    // Prellenar datos de la persona (solo lectura)
                    if (response.persona) {
                        handleChange('nombre', response.persona.nombre || '');
                        handleChange('apellidoPaterno', response.persona.apellido_paterno || '');
                        handleChange('apellidoMaterno', response.persona.apellido_materno || '');
                        handleChange('correo', response.persona.correo_electronico_personal || '');
                        handleChange('telefono', response.persona.numero_telefono_personal || '');
                    ...
                    showToast('error', 'Usuario ya registrado',
                    ...
                    setTimeout(() => {
                        navigate('/login', {
                            state: {
                                message: 'Ya tienes una cuenta registrada. Por favor inicia sesión.',
    ...
    const isPaso1Valido = () => {
        // Si los campos están bloqueados (documento existente sin usuario),
        // solo necesitamos el documento verificado
        if (camposBloqueados) {
            return tipoSeleccionado && formData.numeroDocumento.trim() && documentoVerificado;
        }

        // Si es registro nuevo, validar todos los campos
        return (
            tipoSeleccionado &&
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-blue-600 text-xs">
                                                    Tus datos personales han sido obtenidos de nuestro sistema.
                                                    Solo debes crear tu usuario y contraseña en el siguiente paso.
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex flex-col gap-2 md:col-span-2">
                                        ...
                                        <div className="relative">
                                            <InputText
                                                ...
                                                className={`w-full ${!usuarioDisponible && formData.usuario.trim() ? 'p-invalid' : ''}`}
                                            />
                                            {verificandoUsuario && (
                                                <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
                                                    <RotateCw className="w-4 h-4 text-blue-500 animate-spin" />
                                        ...
                                        {formData.usuario.trim() && !verificandoUsuario && !mensajeUsuario && (
                                            <small className="text-gray-500">
                                                Escribe al menos 3 caracteres para verificar disponibilidad
                                            </small>
                                        )}
                                    </div>
                                ...
                                <div className="flex flex-col sm:flex-row w-full justify-center items-center gap-4">
                                    ...
                                    <button
                                        ...
                                        className={`flex gap-2 bg-[#fd4c82] hover:bg-[#e63b6f] border-none text-white items-center justify-center rounded-lg px-5 py-3 ${
                                            !isPaso3Valido() || registrando ? 'opacity-50 cursor-not-allowed' : ''
                                        }`}
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <p className="text-gray-500 text-sm mt-1">
                                            Serás redirigido al login en <span className="font-semibold">3 segundos...</span>
                                        </p>
                                    </motion.div>
